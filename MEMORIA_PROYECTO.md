...
| 2026-06-11 | **Git + coherencia del ciclo (calibración).** Repo conectado por Oscar a `github.com/oscarb92/trading`; completado el versionado (50 archivos, commit `5f4deef`, push a main; `.gitignore` ampliado, `test_run.py` muerto eliminado). **Cierre del pendiente de Fase 3:** `src/calibration.py` (reliability curve + Brier vs base). **Veredicto: las probabilidades del forecast NO están calibradas — skill NEGATIVO** (BTC 1h −12.6%, BTC 4h −23.1%, ETH 1h −18.0%, ETH 4h −30.1%; la frecuencia observada se queda en ~50% diga lo que diga el modelo). La pestaña Ciclo ahora lo declara (banner "demo de simulación" + expander con la tabla de calibración; verificado en navegador). Implicación: jamás usar esas probabilidades para sizing/Kelly. 40 tests verdes. |
| 2026-06-11 | **Paper trading VIVO + skills actualizadas.** Tarea programada de Windows **"TradingApp-PaperCycle"** (cada hora, `run_cycle.bat` → `logs/cycle.log`; gestionable en `taskschd.msc` o `schtasks /Change /TN "TradingApp-PaperCycle" /Disable|/Enable`). Saldo simulado 1.000 USDT depositado, `enabled: true`, modo `auto_testnet`; primera pasada verificada (precio real ETH, decisión "mantener"). **Las 10 skills de `skills/` actualizadas** al código real (store/marketdata/validation/calibration, 5 familias de señal, tarea programada, veredictos OOS y calibración negativa, auto_live congelado). Pausar el paper: kill-switch del dashboard o deshabilitar la tarea. |
| 2026-06-11 | **Cambio a ciclo EN-APP (decisión de Oscar: evitar consumo con la app cerrada).** Eliminada la tarea de Windows y `run_cycle.bat`; sustituidos por un `@st.fragment(run_every="60s")` en la pestaña Ciclo con toggle "Ciclo automático mientras la app esté abierta" + intervalo configurable (persistido en `schedule.app_auto`/`app_every_min`; marca anti-dobles en `state/last_cycle.json`). Verificado en navegador: primera pasada inmediata al activar, countdown correcto, y **respeta el kill-switch** (corrió con enabled=false y reportó "Automatización deshabilitada" sin operar). Pausar: toggle, kill-switch o cerrar la app. Skills schedule-control/auto-trader/paper-trade actualizadas. |
| 2026-06-11 | **Rediseño UX (petición de Oscar).** (1) Nueva pestaña **⚙️ Configuración** con formulario único (modo, frecuencia, ciclo en-app, símbolos, confianza, límites de riesgo); el **cron desaparece de la UI** sustituido por selector amigable ("Cada 15 min/Cada hora/…"); el sidebar queda como panel de **Estado** + kill-switch de emergencia (guarda al vuelo). (2) Gráficas estilo app de mercado con **Plotly**: pestaña Mercado con **velas japonesas + volumen** (verde/rojo, botones de rango 1d/1sem/1mes/6m/todo, precio en vivo para cripto, multi-mercado) y equity/drawdown del backtest con zoom de barra de herramientas; `scrollZoom` desactivado (la rueda no mueve las gráficas). plotly añadido a requirements. Verificado en navegador. 40 tests verdes. |
| 2026-06-11 | **Navegación por menú lateral (petición de Oscar).** Las 9 pestañas horizontales pasan a un **menú en el sidebar** (radio): solo se renderiza la sección activa (más rápido y sin perderse). El sidebar queda: logo + menú + Estado + kill-switch. El **tick del ciclo en-app se movió a global** (corre en cualquier sección, no solo en Ciclo; su estado se muestra bajo las métricas en todas las páginas). Verificado en navegador (navegación + tick global "próxima pasada en ~N min"). 40 tests verdes. |
| 2026-06-11 | **FUTUROS PAPER completos (petición de Oscar).** Nuevo `src/futures.py`: `liquidation_price`, funding prorrateado y `simulate_futures` (margen cruzado de cuenta completa, **liquidación INTRABAR** por mecha adversa). Integración en vivo: `risk.size_position(leverage)`, `Position` con leverage/liq_price/last_funding (retro-compatible), `PaperBroker.open(leverage)` calcula liq, `engine` cobra funding por ciclo y **liquida forzosamente** (nota LIQUIDACION en journal). Config `futures:` (enabled/leverage≤5/funding/maint). UI: sección Futuros en ⚙️, slider de apalancamiento en Backtest (banner 💀 si liquida), columna ⚠ Liq. en Portfolio, calculadora de liquidación en Riesgo. **Medición real (`futures_compare.py` → `reports/futures.md`): BTC histéresis −23% a 1× → −99% a 5×; ETH B&H a 5× LIQUIDADO (−100%). Sin edge, el apalancamiento solo decide cuándo pierdes.** Verificado en navegador (BTC 4h 5×: −99.97%). 47 tests verdes (+7). |
| 2026-06-12 | **RE-calibración del forecast (Platt scaling) — pregunta de Oscar "¿cómo calibramos?".** `calibration.platt_fit` (logística 1D por Newton, sin sklearn) + `walkforward_calibration` (ajusta a,b en la 1ª mitad, evalúa en la 2ª; sin look-ahead). **Medición real: skill crudo −12%…−30% → calibrado ≈0 (+0.1%); el coeficiente aprendido a≈−0.05 (≈cero) y TODAS las probabilidades calibradas colapsan a ~50%±1.7%.** Lección central documentada en la UI (expander del Ciclo): calibrar repara la mentira pero no crea conocimiento — un ciclo que usara la probabilidad calibrada no operaría nunca, y esa es la recomendación honesta del forecast. También verificado que Platt CONSERVA la discriminación cuando sí hay señal (test sintético, skill OOS >20%). 50 tests verdes (+3). Primer futuro paper vivo validado (BTC long 3×, liq −33%, funding corriendo). |
| 2026-06-12 | **Tick ADAPTATIVO (idea de Oscar).** Con posiciones abiertas el ciclo en-app corre al ritmo de vigilancia (`schedule.app_every_min_active`, default 5 min, configurable 1-15 en ⚙️); sin posiciones, al ritmo normal. Motivo: el broker paper NO es un exchange 24/7 — stops/TP/liquidación solo se evalúan en cada pasada, y con apalancamiento 30 min entre ticks era demasiado. La línea de estado muestra "👁 VIGILANCIA: posición abierta → cada N min". Verificado en vivo (cambió solo de 30→5 min al detectar las posiciones BTC long 3× y ETH short 3×). 50 tests verdes. |
| 2026-06-12 | **Pronóstico de VOLATILIDAD (EWMA/RiskMetrics) — la mejora que faltaba.** Tras la lección "dirección: skill≈0", se midió con la misma vara lo que SÍ se predice: `src/volforecast.py` (`ewma_vol` causal + `vol_skill` con base constante, evaluado en 2ª mitad). **Resultado real: skill de vol +11.0% (BTC 1h, corr +0.32), +7.1% (BTC 4h), +3.8%/+1.1% (ETH)** — positivo y genuino donde la dirección daba 0. `forecast.predict` ahora usa la EWMA (mejores stops/sizing: predecir el riesgo, no el rumbo) y el expander de calibración muestra el contraste direccional≈0 vs vol>0. 53 tests verdes (+3). |
